...
const ManagerLogin = () => {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      console.log("Decoded Token for manager:", decodedToken);

      //   // console.log("Fetched Manager Data:", manager.data);
      if (decodedToken.user_id) {
        Cookies.set("manager_id", decodedToken.user_id, {
          secure: true,
          sameSite: "strict",
        });
      }
      const manager = await api.get(`/users/managers/${decodedToken.user_id}/`);
      if (manager.data && manager.data.id) {
        Cookies.set(decodedToken.user_id, JSON.stringify(manager.data), {
          secure: true,
          sameSite: "strict",
        });
      }
      ...
      // Navigate to manager dashboard after successful login
      setTimeout(() => {
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-secondary-50 via-primary-50 to-secondary-100 p-4">
      <Card className="w-full max-w-md shadow-2xl">
        <CardHeader className="flex flex-col gap-3 items-center pt-8 pb-4">
          <div className="flex items-center justify-center w-16 h-16 rounded-full bg-gradient-to-br from-secondary-500 to-primary-500">
            <FontAwesomeIcon
              icon={faUserTie}
